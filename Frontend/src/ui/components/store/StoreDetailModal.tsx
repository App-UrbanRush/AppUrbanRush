...
const StoreDetailModal = ({ store, onClose }: StoreDetailModalProps) => {
  ...
  const photos = [store.storefront_image_url, store.logo_url, store.image].filter((p): p is string => !!p);
  if (photos.length === 0) photos.push(store.image);
  ...
  const baseRating = store.rating ?? 0;
  const avg = storeReviews.average(store.id, baseRating);
      ...
      <div className="store-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="store-modal-body">
          ...
          <div className="store-modal-meta">
            ...
            {store.deliveryTime && <span><Clock size={15} /> {store.deliveryTime}</span>}
          </div>

          <h3 className="store-modal-menu-title">Menú</h3>
          <div className="store-modal-menu">
            {store.products && store.products.length > 0 ? store.products.map((p) => (
              <div className="store-menu-item" key={p.product_id}>
              ...
              <p style={{ color: "#888", fontSize: 14, padding: "12px 0" }}>Este negocio aún no tiene productos publicados.</p>
            )}
          </div>
